...
void MaquinaDeEstados::DePie()
{
    ...
    // aquí para evitar tocar flags incoherentes).
    if (m_lockAtaque && estado_ == m_estadoBloqueado) {
        return;
    }

    // Si ya está de pie, no hacemos trabajo extra
    if (m_isDePie) {
        return;
    }
    ...
    // Pedido de cambio de estado a QUIETO (usará la misma puerta de entrada)
    cambiar(Estado::QUIETO);
}
